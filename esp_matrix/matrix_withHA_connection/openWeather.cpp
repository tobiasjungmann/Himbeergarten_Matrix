...
#define MAX_TEMP_AGE 60
    ...
    void showTemperature() {
        if (outsideTempAge >= MAX_TEMP_AGE) {
            String serverPath =
                    String("http://api.openweathermap.org/data/2.5/weather?q=") + String(OPEN_WEATHER_CITY) +
                    String(",") + String(OPEN_WEATHER_COUNTRY_CODE) + String("&APPID=") + String(OPEN_WEATHER_API_KEY) +
                    String("&units=metric");
